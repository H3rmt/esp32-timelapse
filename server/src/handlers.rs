use crate::consts::{FOLDER_NAME, LAYER_NAME, MINUTE_NAME};
use crate::export;
use axum::body::Bytes;
...
#[tracing::instrument]
pub async fn start() -> Result<String, StatusCode> {
...
pub async fn upload(Query(params): Query<UploadParams>, body: Bytes) -> impl IntoResponse {
...
#[tracing::instrument]
pub async fn finish(Query(params): Query<FinishParams>) -> impl IntoResponse {
    let identifier = params.identifier.ok_or_else(|| {
        warn!("Error: Identifier param not found");
        StatusCode::BAD_REQUEST
    })?;
...
#[tracing::instrument]
pub async fn latest_image(Query(params): Query<LatestParams>) -> impl IntoResponse {
    let identifier = if let Some(id) = params.identifier {
        ...
        for entry in read_root {
            ...
            if last_id.as_ref().is_none_or(|f| name > *f) {
                last_id = Some(name);
            }
        }
    ...
    for entry in read_dir {
        ...
        let name = entry.file_name().to_string_lossy().to_string();
        if name.starts_with(&format!("{img_type}_"))
            && name.ends_with(".jpg")
            && latest_file.as_ref().is_none_or(|f| name > *f)
        {
...
#[tracing::instrument]
pub async fn final_video(Query(params): Query<LatestParams>) -> impl IntoResponse {
    let identifier = if let Some(id) = params.identifier {
        id
    } else {
        let mut last_id: Option<String> = None;
    ...
    for entry in read_dir {
        ...
        let name = entry.file_name().to_string_lossy().to_string();
        if name.starts_with(video_name)
            && name.ends_with(".mp4")
            && latest_file.as_ref().is_none_or(|f| name > *f)
        {
            latest_file = Some(name);
